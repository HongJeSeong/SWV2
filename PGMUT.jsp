		...
		<%@ page import = "java.sql.*" %>
                ...
                <%@ page import = "java.util.ArrayList" %>
                        ...
                        try{
                                ...
                                String sql1 = "DELETE FROM artifacts_info WHERE project_id="+proj_name+" and artifact_name like 'PGM%';";
                                String sql2 = "DELETE FROM artifacts_info WHERE project_id="+proj_name+" and artifact_name like 'UT%';";
                                String sql3 = "SELECT a.project_id, b.customized_id, b.value FROM issues AS a, custom_values AS b WHERE a.project_id="+proj_name+" and a.id=b.customized_id and b.value LIKE 'PGM%';";
                                String sql4 = "SELECT a.project_id, b.customized_id, b.value FROM issues AS a, custom_values AS b WHERE a.project_id="+proj_name+" and a.id=b.customized_id and b.value LIKE 'UT%';";
                        ...
                        int value_count = 0;
                        int value_count2 = 0;
                        ArrayList<String> name_pgm = new ArrayList<String>();
                        ...
                        while(rs.next()){
                                name_pgm.add(rs.getString("artifact_name"));
                                value_count++;
                        }
                        ...
                        Collections.sort(name_pgm);
                        Collections.sort(name_ut);
                        out.println("<form action='PGMUTModify.jsp?proj_name="+proj_name+"' method='post'><table cellpadding='5' cellspacing='0' border='1' style='border-style: solid; border-width: 1px; top: 80px; left: 50px; position: absolute; text-align: center; border-color:black; border-radius: 5px;'>");
                        String from="";
                        String to="";
                        for(int i=0; i<=value_count;i++){
                                ...
                                for(int j=0;j<=value_count2; j++){
                                        if(i==0&&j==0){
                                                ...
                                                out.println("<td>"+name_pgm.get(i-1)+"</td>");
                                        }else if(i==0&&j!=0){
                                                out.println("<td>"+name_ut.get(j-1)+"</td>");
                                        }else{
                                                pstmtTrace = connTrace.prepareStatement("SELECT artifact_id FROM artifacts_info WHERE artifact_name ='"+name_pgm.get(i-1)+"' and project_id="+proj_name+";");
                                                rs = pstmtTrace.executeQuery();
                                                while(rs.next()){
                                                        from=rs.getString("artifact_id");
                                                }
                                                pstmtTrace = connTrace.prepareStatement("SELECT artifact_id FROM artifacts_info WHERE artifact_name ='"+name_ut.get(j-1)+"' and project_id="+proj_name+";");
                                                rs = pstmtTrace.executeQuery();
                                                while(rs.next()){
